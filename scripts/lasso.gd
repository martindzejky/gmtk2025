...
func _draw():
  if has_hook() and not player.cutscene and not player.dead and not player.dying:
...
func _process(_delta):
  ...
  if Input.is_action_just_pressed('shoot') and not player.cutscene and not player.dead and not player.dying:
    if has_hook():
